...
all_dat <- all_dat %>% group_by(Hugo_Symbol) %>% summarise_all(median) # # group data by gene and calculate mean for all the other columns 
...
for(i in 2:length(human_expre)){
  ...
  dat1 <- dat %>% group_by(Hugo_Symbol) %>% summarise_all(median)  # group data by gene and calculate mean for all the other columns
